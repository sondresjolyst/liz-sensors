...
def main():
    ...
    if not vault_github_token:
        vault_github_token = getpass.getpass(
            "Enter your GitHub token for Vault authentication: "
        )
    ...
    try:
        ...
        if not mac_match:
            raise Exception(
                f"No MAC address found in esptool output: '{result.stdout}'"
            )
    ...
    try:
        ...
        if (
            existing_secret
            and existing_secret.get("data")
            and existing_secret["data"].get("data")
        ):
            print(f"Credentials already exist in Vault for {sensor_name}.")
            choice = input("Use existing credentials? (y/n): ").strip().lower()
            if choice == "y":
                mqtt_password = existing_secret["data"]["data"][
                    vault_secret_password_key
                ]
                ...
                client.secrets.kv.v2.create_or_update_secret(
                    path=vault_path,
                    secret={
                        vault_secret_password_key: mqtt_password,
                        vault_secret_username_key: sensor_name,
                    },
                    mount_point=vault_kv_name,
                )
                action = "updated"
                print(
                    f"New password for {sensor_name} stored in Vault at {vault_kv_name}/{vault_path}"
                )
        else:
            mqtt_password = secrets.token_urlsafe(16)
            client.secrets.kv.v2.create_or_update_secret(
            ...
            action = "created"
            print(
                f"Password for {sensor_name} stored in Vault at {vault_kv_name}/{vault_path}"
            )
    except Exception as e:
    ...
    sensor_acl = [
        {
            "action": "all",
            "permission": "allow",
            "topic": f"garge/devices/{garge_name}_{mac}/#",
            "qos": 0,
            "retain": 1,
        },
        {
    ...
    try:
        ...
        create_or_update_mqtt_user(
            db_connection, database_user_table, sensor_name, salt, password_hash, False
        )

        set_mqtt_acl(db_connection, database_acl_table, sensor_name, sensor_acl)
